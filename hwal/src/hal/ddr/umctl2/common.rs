...
pub trait Umctrl2MPhyV2:DdrDriver {
    ...
    fn enter_phy_init(&self) -> (u32, u32){
        ...
        self.clr_ctrl_reg(UDDRC_PWRCTL, UDDRC_PWRCTL_POWERDOWN_EN | UDDRC_PWRCTL_SELFREF_EN | UDDRC_PWRCTL_EN_DFI_DRAM_CLK_DISABLE);
    ...
    fn wait_init_done(&self, pwrctl:u32, rfshctl3:u32) {
        ...
        loop {
            if self.read_ctrl_reg(UDDRC_STAT) & UDDRC_STAT_OPERATING_MODE_MSK == UDDRC_STAT_OPERATING_MODE_NORMAL {
                break
            }
        }
        // ----DWC_ddr_umctl2 and Memory Initialization with LPDDR4 mPHY_v2:step 26
        //recovery pwrctl and rfshctl3
        self.write_ctrl_reg(UDDRC_PWRCTL, pwrctl);
    ...
    fn init_umctrl2(&self) {
        ...
        if self.skip_train() {
            ...
            self.clr_ctrl_reg(UDDRC_PWRCTL, UDDRC_RFSHCTL3_DIS_AUTO_REFRESH);
        } else {
            self.set_ctrl_reg(UDDRC_INIT0, UDDRC_INIT0_SKIP_DRAM_INIT_MSK);
            self.set_ctrl_reg(UDDRC_PWRCTL, UDDRC_RFSHCTL3_DIS_AUTO_REFRESH);
        }
