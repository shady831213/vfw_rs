...
fn hello_world() -> u32 {
    let hartid = hartid();
    try_fork(task3 as usize, &[hartid]).unwrap();
    let task1_id = try_fork(task1 as usize, &EMPTY_ARGS).unwrap();
    let task2_id = fork_on(
        1,
        task2 as usize,
        &[hartid, task3 as usize, &100usize as *const usize as usize],
    );
    join(task1_id);
    for _ in 0..10 {
        print!("Hello ");
